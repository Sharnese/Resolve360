...
const StatementSubmission: React.FC = () => {
  ...
  const submitStatement = async (e: React.FormEvent) => {
    ...
    // Notify admin that a witness statement was submitted.
    sendNotification('witness_statement_submitted', stmt.request_id, {
      statementId: stmt.id,
      witnessName: `${firstName || stmt.first_name || ''} ${lastName || stmt.last_name || ''}`.trim(),
    });
    setSubmitted(true);
    setSubmitting(false);
  };
